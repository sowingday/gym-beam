...
import { deriveFriends, fetchFollows, isSocialAvailable, sendInboxShare } from '../lib/socialService';
...
export default function ShareAchievementsDialog({ period, initialTab = 'extern', buildShareText, onClose }) {
  ...
  const [tab, setTab] = useState(initialTab === 'freunde' && !socialAvailable ? 'extern' : initialTab);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl p-5 w-80 max-w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex rounded-lg overflow-hidden border border-border mb-4">
          ...
          <button
            onClick={() => socialAvailable && setTab('freunde')}
            disabled={!socialAvailable}
            className={`flex-1 py-1.5 text-xs font-body flex items-center justify-center gap-1 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${tab === 'freunde' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted/50'}`}
          >
